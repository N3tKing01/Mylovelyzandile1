...
function linkPreview(options={}) {
	...
	const opt = {
	       showAdAttribution: true,
               title: options.title || config.LINK_PREVIEW.split(/[;,|]/)[0] || 'BOYKA-MK',
               body:  options.body || config.LINK_PREVIEW.split(/[;,|]/)[1],
               mediaType: 1,
               thumbnailUrl: options.url || config.LINK_PREVIEW.split(/[;,|]/)[2] || 'https://files.catbox.moe/4qwyn2.jpeg',
               sourceUrl: config.LINK_PREVIEW.split(/[;,|]/)[3] || 'https://whatsapp.com/channel/0029VaraMtfFcowAKRdDdp1T'
             }
             return opt
}
...
async function AudioMetaData(audio, info = {}) {
	...
	let img = info.image || 'https://i.postimg.cc/4NdSqms8/MidKing.jpg';
