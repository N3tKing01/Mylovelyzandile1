...
plugin({
    ...
    try {
        ...
        let packname = match || config.STICKER_DATA.split(/[|;,]/)[1] || 'BREAL-MD';
        let author = config.STICKER_DATA.split(/[|;,]/)[0] || 'BREAL';
...
plugin({
    ...
    try {
        ...
        let packname = metadata[0] || config.STICKER_DATA.split(/[|;,]/)[1] || 'BREAL-MD';
        let author = metadata[1] || config.STICKER_DATA.split(/[|;,]/)[0] || 'BREAL';
...
plugin({
    ...
    try {
        ...
        let packname = match || 'Stolen by BREAL-MD';
        let author = config.STICKER_DATA.split(/[|;,]/)[0] || 'BREAL';
    ...
    try {
        if (match) {
            ...
            if (newData.length >= 2) {
                ...
                await message.send('📦 *Sticker Pack Setup*\n\nUsage: .stickerpack author|packname\nExample: .stickerpack BREAL|My Cool Stickers');
            }
        } else {
            let currentAuthor = config.STICKER_DATA.split(/[|;,]/)[0] || 'BREAL';
            let currentPack = config.STICKER_DATA.split(/[|;,]/)[1] || 'BREAL-MD';
